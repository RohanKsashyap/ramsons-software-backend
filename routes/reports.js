...
router.post('/sales', async (req, res) => {
  try {
    ...
    // Fetch invoice transactions directly from database
    ...
    const salesData = transactions.reduce((acc, t) => {
      acc.totalSales += parseFloat(t.paidAmount || 0);
      acc.totalInvoiced += parseFloat(t.amount || 0);
      acc.totalOutstanding += parseFloat((t.amount - (t.paidAmount || 0)) || 0);
      return acc;
    }, { totalSales: 0, totalInvoiced: 0, totalOutstanding: 0 });
